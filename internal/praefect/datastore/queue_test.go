...
func TestPostgresReplicationEventQueue_GetUpToDateStorages(t *testing.T) {
	...
	noStorages, err := queue.GetUpToDateStorages(ctx, eventType1.Job.VirtualStorage, eventType1.Job.RelativePath)
	require.NoError(t, err)
	require.Empty(t, noStorages, "no up to date storages expected as database is empty")
	...
	noStorages, err = queue.GetUpToDateStorages(ctx, eventType1.Job.VirtualStorage, eventType1.Job.RelativePath)
	require.NoError(t, err)
	require.Empty(t, noStorages, "no up to date storages as job is in 'ready' state")

	devents1, err := queue.Dequeue(ctx, event1.Job.VirtualStorage, event1.Job.TargetNodeStorage, 100500)
	require.NoError(t, err)
	require.Len(t, devents1, 1, "exactly one event expected as only one was queued")

	noStorages, err = queue.GetUpToDateStorages(ctx, devents1[0].Job.VirtualStorage, devents1[0].Job.RelativePath)
	require.NoError(t, err)
	require.Empty(t, noStorages, "no up to date storages as job is in 'in_progress' state")

	acks1, err := queue.Acknowledge(ctx, JobStateFailed, []uint64{devents1[0].ID})
	require.NoError(t, err)
	require.Equal(t, []uint64{devents1[0].ID}, acks1)

	noStorages, err = queue.GetUpToDateStorages(ctx, devents1[0].Job.VirtualStorage, devents1[0].Job.RelativePath)
	require.NoError(t, err)
	require.Empty(t, noStorages, "no up to date storages as job is in 'failed' state")

	devents1, err = queue.Dequeue(ctx, event1.Job.VirtualStorage, event1.Job.TargetNodeStorage, 100500)
	require.NoError(t, err)
	require.Len(t, devents1, 1, "exactly one event expected as only one was queued and there is more attempt to process it")

	acks1, err = queue.Acknowledge(ctx, JobStateCompleted, []uint64{devents1[0].ID})
	require.NoError(t, err)
	require.Equal(t, []uint64{devents1[0].ID}, acks1)

	storages, err := queue.GetUpToDateStorages(ctx, devents1[0].Job.VirtualStorage, devents1[0].Job.RelativePath)
	require.NoError(t, err)
	require.Equal(t, []string{eventType1.Job.TargetNodeStorage}, storages)
}
